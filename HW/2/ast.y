...
%token READ WRITE IF ELSE WHILE DO FOR INT FLOAT OR AND NOT IMPLY SWITCH CASE DEFAULT BREAK
...
do_while_stmt: DO stmt WHILE '(' boolexp ')'  ';' { $$ = new DoWhileStmt($5, $2); }

switch_stmt : SWITCH '(' expression ')' '{' caselist DEFAULT ':' stmt '}' {
  if($3->_type != _INT) {
    errorMsg("line %d: Expression in switch statement must evaluate to integer\n", @1.first_line);
  }
  $$ = new SwitchStmt ($3, $6, $9, @1.first_line);
};
