...
void Or::genBoolExp (int truelabel, int falselabel)
{
    ...
    if  (truelabel == FALL_THROUGH) {
	    ...
	    _left->genBoolExp (next_label, // if left operand is true then the OR expression
		                               //is true so jump to next_label (thus falling through to the code following the code for the OR expression)
...
void DoWhileStmt::genStmt()
{
  ...
  _body->genStmt ();
	...
	_condition->genBoolExp (startLabel, FALL_THROUGH);
  emit ("label%d:\n", exitLabel);
...
void SwitchStmt::genStmt()
{
   ...
   _exp->genExp();
    std::queue<int> caseLabels;
    for (Case *myCase = _caselist; myCase != NULL; myCase = myCase->_next){
        int condlabel = newlabel ();
        caseLabels.push(condlabel);
        emit ("if _t%d == %d goto label%d\n", _exp->_result, myCase->_number, condlabel);
    }
